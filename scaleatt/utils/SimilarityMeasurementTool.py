...
class SimilarityMeasurementTool:
    ...
    def sim_measure(img1: np.ndarray, img2: np.ndarray, similarity_measurement: SimilarityMeasure):
        ...
        elif similarity_measurement == SimilarityMeasure.SSIM:
            simscore = skimage.measure.compare_ssim(img1, img2, multichannel=True, gaussian_weights=True, sigma=1.5)
        elif similarity_measurement == SimilarityMeasure.MSE:
            simscore = SimilarityMeasurementTool.mse(img1, img2)
        else:
    ...
    def mse(img1, img2):
        ...
        err /= float(img1.size)
        return 1/err # convert again by dividing, so that high MSE == high dissimilarity gets low similarity score.
